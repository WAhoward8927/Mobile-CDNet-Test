...
from models.model_v4 import BaseNetV4
...
def main(args):
    ...
    run_dir = Path(args.output_root) / f"BCDD_V4_seed{args.seed}"
    ...
    config["variant"] = "V4_shallow_boundary_spatial_gate"
    config["v4_changes"] = {
        "shallow_boundary_branch": "c1_abs_difference_plus_local_high_frequency",
        "spatial_difference_attention": ["c2", "c3"],
        "attention_kernel": 7,
        "refinement": "two_stage_depthwise_separable_residual",
        "loss_and_training_recipe": "unchanged_V1",
    ...
    model = BaseNetV4(pretrained=True).cuda()
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            if args.gate_epoch > 0 and epoch + 1 == args.gate_epoch and best_f1 < args.gate_min_f1:
                gate_report = {
                    ...
                    "reason": "V4 did not reach the pre-registered 40-epoch validation gate",
                }
                (run_dir / "early_stop.json").write_text(json.dumps(gate_report, indent=2), encoding="utf-8")
                print("V4_GATE_EARLY_STOP " + json.dumps(gate_report), flush=True)
                break

    ema_model.load_state_dict(torch.load(run_dir / "best_model.pth", map_location="cuda", weights_only=False))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gate_epoch", type=int, default=40)
    parser.add_argument("--gate_min_f1", type=float, default=0.9570)
