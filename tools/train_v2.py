...
class TemporalPhotometricJitterV2:
    """Weak paired illumination jitter plus small independent sensor noise."""

    def __init__(self, probability=0.8, gain=0.08, bias=6.0, noise_probability=0.5, noise_std=1.5):
    ...
    def __call__(self, image, label):
        ...
        if random.random() < self.probability:
            shared_gain = np.random.uniform(1.0 - self.gain, 1.0 + self.gain)
            shared_bias = np.random.uniform(-self.bias, self.bias)
            image = image * shared_gain + shared_bias
        if random.random() < self.noise_probability:
            image[:, :, :3] += np.random.normal(0.0, self.noise_std, image[:, :, :3].shape).astype(np.float32)
            image[:, :, 3:6] += np.random.normal(0.0, self.noise_std, image[:, :, 3:6].shape).astype(np.float32)
        return [np.clip(image, 0.0, 255.0), label]
...
def segmentation_loss(logits, targets):
    ...
    return 0.4 * bce + 0.3 * (1.0 - dice.mean()) + 0.3 * (1.0 - tversky.mean())
...
def build_change_aware_sampler(dataset, seed):
    ...
    if empty:
        weights[empty] = 0.35 / len(empty)
    changed_scores = np.asarray([area_score[i] for i in changed], dtype=np.float64)
    if changed:
        weights[changed] = 0.65 * changed_scores / changed_scores.sum()
    ...
    return sampler, {
        ...
        "empty": len(empty),
        "changed": len(changed),
        "empty_sampling_mass": 0.35,
        "changed_sampling_mass": 0.65,
    }
...
def main(args):
    ...
    run_dir = Path(args.output_root) / f"BCDD_V2_seed{args.seed}"
    ...
    config["variant"] = "V2_tversky_temporal_jitter_change65"
    config["v2_changes"] = {
        "loss": "0.4_BCE+0.3_Dice+0.3_Tversky",
        "tversky_alpha_fp": 0.35,
        "tversky_beta_fn": 0.65,
        "empty_sampling_mass": 0.35,
        "changed_sampling_mass": 0.65,
        "paired_gain": 0.08,
        "paired_bias": 6.0,
        "independent_noise_std_255": 1.5,
    ...
    ema_model.load_state_dict(torch.load(run_dir / "best_model.pth", map_location="cuda", weights_only=True))
