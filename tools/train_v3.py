...
def auxiliary_segmentation_loss(logits, targets):
    bce = F.binary_cross_entropy_with_logits(logits, targets)
    return 0.5 * bce + 0.5 * dice_loss(logits, targets)


def ohem_bce_loss(logits, targets, negative_fraction=0.20):
    pixel_loss = F.binary_cross_entropy_with_logits(logits, targets, reduction="none")
    positive = pixel_loss[targets > 0.5]
    negative = pixel_loss[targets <= 0.5]
    selected = []
    if positive.numel():
        selected.append(positive)
    if negative.numel():
        hard_count = max(1, int(negative.numel() * negative_fraction))
        selected.append(torch.topk(negative, k=hard_count, sorted=False).values)
    if not selected:
        return pixel_loss.mean()
    return torch.cat(selected).mean()


def soft_boundary(values, kernel_size=3):
...
def boundary_dice_loss(logits, targets):
    pred_boundary = soft_boundary(torch.sigmoid(logits))
    target_boundary = soft_boundary(targets)
    dims = (1, 2, 3)
    intersection = (pred_boundary * target_boundary).sum(dims)
    denominator = pred_boundary.sum(dims) + target_boundary.sum(dims)
...
def segmentation_loss(logits, targets):
    return (
        0.40 * ohem_bce_loss(logits, targets, negative_fraction=0.20)
        + 0.40 * dice_loss(logits, targets)
        + 0.20 * boundary_dice_loss(logits, targets)
    )
...
def multi_output_loss(outputs, targets, aux_weights=(0.4, 0.2, 0.1)):
    ...
    for weight, aux_logits in zip(aux_weights, aux):
        loss = loss + weight * auxiliary_segmentation_loss(aux_logits, targets)
    return loss
...
def main(args):
    ...
    run_dir = Path(args.output_root) / f"BCDD_V3_seed{args.seed}"
    ...
    config["variant"] = "V3_ohem20_dice_boundary20"
    config["v3_changes"] = {
        "main_loss": "0.40_OHEM-BCE+0.40_per-image-Dice+0.20_Boundary-Dice",
        "ohem_negative_fraction": 0.20,
        "boundary_kernel_size": 3,
        "auxiliary_loss": "unchanged_V1_0.50_BCE+0.50_per-image-Dice",
        "inference_architecture_changed": False,
        "inference_params_added": 0,
        "inference_flops_added": 0,
        "gate_epoch": args.gate_epoch,
        "gate_min_validation_f1": args.gate_min_f1,
        "v1_validation_best_f1": 0.9548905491828918,
    }
    ...
    with log_path.open("a" if args.resume else "w", encoding="utf-8") as log:
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            if args.gate_epoch > 0 and epoch + 1 == args.gate_epoch and best_f1 < args.gate_min_f1:
                gate_report = {
                    "status": "early_stopped_at_gate",
                    "gate_epoch": args.gate_epoch,
                    "best_validation_f1": float(best_f1),
                    "required_validation_f1": float(args.gate_min_f1),
                    "reason": "V3 did not exceed V1 by the required 0.5 percentage points",
                }
                (run_dir / "early_stop.json").write_text(json.dumps(gate_report, indent=2), encoding="utf-8")
                print("V3_GATE_EARLY_STOP " + json.dumps(gate_report), flush=True)
                break
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gate_epoch", type=int, default=100)
    parser.add_argument("--gate_min_f1", type=float, default=0.9598905491828918)
