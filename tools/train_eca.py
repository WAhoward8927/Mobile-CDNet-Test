...
from models.model_eca import BaseNetECA
...
def main(args):
    ...
    run_dir = Path(args.output_root) / f"BCDD_V1_ECA_seed{args.seed}"
    ...
    config["variant"] = "V1_plus_residual_ECA_difference_gates"
    config["eca"] = {
        "location": "five absolute-difference feature scales before PMDA aggregation",
        "channels": [16, 24, 32, 96, 320],
        "adaptive_kernel_sizes": [3, 3, 3, 3, 5],
        "residual_scale_init": 0.0,
        "seeds": [2333, 2334, 2335],
    }
    ...
    model = BaseNetECA(pretrained=True).cuda()
