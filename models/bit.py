...
class Backbone(nn.Module, KaimingInitMixin):
    def __init__(
        ...
        self.conv_out = Conv3x3(out_ch, out_ch)
        ...
        self.conv_scale5_c5 = nn.Sequential(
            nn.Conv2d(256, out_ch, kernel_size=3, stride=1, padding=1),
        ...
        self.conv_aggregation_s5 = FeatureFusionModule(out_ch * 2, 256, out_ch)
    ...
    def forward(self, x):
        ...
        y1 = self.resnet.layer3(y)
        y2 = self.resnet.layer4(y1)

        c5_s5 = self.conv_scale5_c5(y2)
        c4_s5 = self.conv_scale5_c4(y1)

        s5 = self.conv_aggregation_s5(torch.cat([c4_s5, c5_s5], dim=1), y2)
